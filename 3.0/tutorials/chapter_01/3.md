...
这一目标和一开源 UI 框架 https://github.com/TomSchimansky/CustomTkinter 很像。从名字上来看，`customtkinter` 可以高度自定义你的 `tkinter` 程序，`maliang` 3 的目标也是如此，但这还不够。因为再怎么样，`customtkinter` 的控件都无法脱离 `tkinter` 的真实控件，这就导致控件数量十分有限，那么该怎么解决这个问题呢？
